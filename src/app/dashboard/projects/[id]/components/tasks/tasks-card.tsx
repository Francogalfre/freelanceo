...
const TasksCard = ({ task }: { task: Task }) => {
  ...
  return (
    <div
      className={`py-4 px-6 rounded-xl border border-gray-100 flex items-center gap-4 justify-between transition-all duration-300 ease-in-out hover:bg-gray-200/30 ${
      ...
      <div className="flex items-center gap-4">
        <div className="relative">
          <input
            ...
            disabled={isLoading}
            className="peer h-6 w-6 appearance-none rounded-full border border-slate-300 bg-white shadow transition-all duration-300 ease-in-out cursor-pointer checked:bg-blue-600 checked:border-blue-600 checked:ring-2 checked:ring-offset-1 checked:ring-blue-400 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60"
          />
          {isLoading && (
        ...
        <div>
          <p className={`font-medium ${task.isDone ? "line-through text-gray-500" : ""}`}>{task.title}</p>
